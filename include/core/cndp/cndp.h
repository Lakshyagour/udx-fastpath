...
#define FIB_RULES_TAG \
  "l3fwd-fib-rules" /**< json tag to set up static FIB entries */
...
#define MODE_ACL_STRICT \
  "acl-strict" /**< ACL forwarding with permit list mode */
#define MODE_ACL_PERMISSIVE \
  "acl-permissive"                   /**< ACL forwarding with deny list mode */
...
int parse_args(string, int, struct fwd_info* fwd, void**);
 ...
 */
static inline uint8_t get_app_mode(const char* type) {
  if (type) {
    ...
    if (!strncasecmp(type, MODE_DROP, nlen) || !strncasecmp(type, MODE_RX_ONLY, nlen))
      ...
      cne_printf("      '[cyan]%s|%s[]', '[cyan]%s|%s[]', '[cyan]%s[]', ",
      MODE_DROP, MODE_RX_ONLY, MODE_LB, MODE_LOOPBACK, MODE_TX_ONLY);
      cne_printf("'[cyan]%s[]', '[cyan]%s[]', '[cyan]%s[]', '[cyan]%s[]'\n",
      MODE_FWD, MODE_ACL_STRICT, MODE_ACL_PERMISSIVE, MODE_HYPERSCAN);
    }
  }
 ...
 *   The index value of the mode or UNKNOW value is returned.
 */
static inline uint8_t get_pkt_api(const char* type) {
  ...
  if (type) {
    ...
    if (!strncasecmp(type, XSKDEV_API_NAME, nlen))
      ...
      cne_printf(
      "[yellow]*** [magenta]API [orange]xdpdev [magenta]is deprecated![]\n");
      ret = XSKDEV_PKT_API;
      goto warn;
    } else {
...
warn:
  cne_printf(
  "    [magenta]Known API types[] '[cyan]%s[]' [magenta]and[] '[cyan]%s[]' "
  "[magenta], default[] '[cyan]%s[]'\n",
  XSKDEV_API_NAME, PKTDEV_API_NAME, XSKDEV_API_NAME);
  return ret;
}

static inline uint8_t get_dst_lport(void* data) {
...
static inline void rewrite_dst_mac(void* data, struct ether_addr* dst_mac) {
  struct ether_header* eth       = (struct ether_header*)data;
