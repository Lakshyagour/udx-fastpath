...
portid_t lports[RTE_MAX_ETHPORTS];
...
struct fwd_engine* cur_fwd_eng = &run_to_completion_engine; /**< IO mode by default. */
 ...
 */
uint64_t rss_hf = RTE_ETH_RSS_PROTO_MASK; /* RSS IP by default. */

/*
 * Port topology configuration
 */
uint16_t port_topology = PORT_TOPOLOGY_LOOP; /* Ports are paired by default */
...
(UINT32_C(1) << RTE_ETH_EVENT_INTR_LSC) | (UINT32_C(1) << RTE_ETH_EVENT_QUEUE_STATE) |
(UINT32_C(1) << RTE_ETH_EVENT_INTR_RESET) | (UINT32_C(1) << RTE_ETH_EVENT_IPSEC) |
(UINT32_C(1) << RTE_ETH_EVENT_MACSEC) | (UINT32_C(1) << RTE_ETH_EVENT_INTR_RMV) |
(UINT32_C(1) << RTE_ETH_EVENT_FLOW_AGED) | (UINT32_C(1) << RTE_ETH_EVENT_ERR_RECOVERING) |
(UINT32_C(1) << RTE_ETH_EVENT_RECOVERY_SUCCESS) | (UINT32_C(1) << RTE_ETH_EVENT_RECOVERY_FAILED);
/*
...
static void eth_rx_metadata_negotiate_mp(uint16_t port_id) {
...
static int eth_dev_configure_mp(uint16_t port_id, uint16_t nb_rx_q, uint16_t nb_tx_q, const struct rte_eth_conf* dev_conf) {
  if (is_proc_primary())
    return rte_eth_dev_configure(port_id, nb_rx_q, nb_tx_q, dev_conf);
  return 0;
}
...
static void mempool_free_mp(struct rte_mempool* mp) {
  if (is_proc_primary())
    rte_mempool_free(mp);
}
...
static int eth_event_callback(portid_t port_id, enum rte_eth_event_type type, void* param, void* ret_param);
static void dev_event_callback(const char* device_name, enum rte_dev_event_type type, void* param);
 ...
 */
static void set_default_fwd_lcores_config(void) {
  unsigned int i;
  unsigned int nb_lc;
...
static void set_def_peer_eth_addrs(void) {
  portid_t i;
...
static void set_default_fwd_ports_config(void) {
  portid_t pt_id;
  int i = 0;
...
static int create_extmem(uint32_t nb_mbufs, uint32_t mbuf_sz, struct extmem_param* param, bool huge) {
  ...
  for (pgsz_idx = 0; pgsz_idx < RTE_DIM(pgsizes); pgsz_idx++) {
    ...
    iovas = (rte_iova_t*)malloc(sizeof(*iovas) * n_pages);
...
static int setup_extmem(uint32_t nb_mbufs, uint32_t mbuf_sz, bool huge) {
  ...
  EXTMEM_HEAP_NAME, param.addr, param.len, param.iova_table, param.iova_table_len, param.pgsz);
...
static void dma_unmap_cb(struct rte_mempool* mp __rte_unused,
...
unsigned mem_idx __rte_unused) {
  ...
  if (ret) {
    TESTPMD_LOG(DEBUG, "unable to un-register addr 0x%p\n", memhdr->addr);
  }
}

static void dma_map_cb(struct rte_mempool* mp __rte_unused,
void* opaque __rte_unused,
struct rte_mempool_memhdr* memhdr,
unsigned mem_idx __rte_unused) {
  uint16_t pid     = 0;
  ...
  ret = rte_extmem_register(memhdr->addr, memhdr->len, NULL, 0, page_size);
  if (ret) {
    TESTPMD_LOG(DEBUG, "unable to register addr 0x%p\n", memhdr->addr);
    return;
  }
  RTE_ETH_FOREACH_DEV(pid) {
    struct rte_eth_dev_info dev_info;
...
static unsigned int setup_extbuf(uint32_t nb_mbufs,
uint16_t mbuf_sz,
...
struct rte_pktmbuf_extmem** ext_mem) {
  ...
  xmem = (rte_pktmbuf_extmem*)malloc(sizeof(struct rte_pktmbuf_extmem) * zone_num);
  if (xmem == NULL) {
    TESTPMD_LOG(ERR,
  ...
  for (ext_num = 0; ext_num < zone_num; ext_num++) {
    ...
    ret = snprintf(mz_name, sizeof(mz_name), RTE_MEMPOOL_MZ_FORMAT "_xb_%u", pool_name, ext_num);
    if (ret < 0 || ret >= (int)sizeof(mz_name)) {
      errno   = ENAMETOOLONG;
      ext_num = 0;
      break;
    }
    mz = rte_memzone_reserve(mz_name, EXTBUF_ZONE_SIZE, socket_id,
    RTE_MEMZONE_IOVA_CONTIG | RTE_MEMZONE_1GB | RTE_MEMZONE_SIZE_HINT_ONLY);
    if (mz == NULL) {
      /*
...
mbuf_pool_create(uint16_t mbuf_seg_size, unsigned nb_mbuf, unsigned int socket_id, uint16_t size_idx) {
  ...
  if (!is_proc_primary()) {
    ...
    if (rte_mp == NULL)
      rte_exit(EXIT_FAILURE, "Get mbuf pool for socket %u failed: %s\n", socket_id, rte_strerror(rte_errno));
    return rte_mp;
  }
...
#endif
  if (rte_mp == NULL) {
    rte_exit(EXIT_FAILURE, "Creation of mbuf pool for socket %u failed: %s\n", socket_id,
 ...
 */
static int check_socket_id(const unsigned int socket_id) {
...
int check_nb_txq(queueid_t txq) {
  ...
  if (txq > allowed_max_txq) {
    fprintf(stderr, "Fail: input txq (%u) can't be greater than max_tx_queues (%u) of port %u\n",
 ...
 */
static uint16_t get_allowed_max_nb_rxd(portid_t* pid) {
 ...
 */
static uint16_t get_allowed_min_nb_rxd(portid_t* pid) {
...
int check_nb_rxd(queueid_t rxd) {
  ...
  if (rxd < allowed_min_rxd) {
    fprintf(stderr, "Fail: input rxd (%u) can't be less than min_rxds (%u) of port %u\n", rxd,
    allowed_min_rxd, pid);
    return -1;
  }
 ...
 */
static uint16_t get_allowed_max_nb_txd(portid_t* pid) {
 ...
 */
static uint16_t get_allowed_min_nb_txd(portid_t* pid) {
...
int check_nb_txd(queueid_t txd) {
  ...
  if (txd > allowed_max_txd) {
    fprintf(stderr, "Fail: input txd (%u) can't be greater than max_txds (%u) of port %u\n", txd,
    allowed_max_txd, pid);
    return -1;
  }
  ...
  if (txd < allowed_min_txd) {
    fprintf(stderr, "Fail: input txd (%u) can't be less than min_txds (%u) of port %u\n", txd,
    allowed_min_txd, pid);
    return -1;
  }
  return 0;
...
int check_nb_hairpinq(queueid_t hairpinq) {
  ...
  if (hairpinq > allowed_max_hairpinq) {
    fprintf(stderr, "Fail: input hairpin (%u) can't be greater than max_hairpin_queues (%u) of port %u\n",
...
static void init_config_port_offloads(portid_t pid, uint32_t socket_id) {
  ...
  if (max_rx_pkt_len)
    port->dev_conf.rxmode.mtu = max_rx_pkt_len - get_eth_overhead(&port->dev_info);
   ...
   */
  if (port->dev_info.rx_desc_lim.nb_mtu_seg_max != UINT16_MAX && port->dev_info.rx_desc_lim.nb_mtu_seg_max != 0) {
    uint32_t eth_overhead = get_eth_overhead(&port->dev_info);
    uint16_t mtu;

    if (rte_eth_dev_get_mtu(pid, &mtu) == 0) {
      uint16_t data_size   = (mtu + eth_overhead) / port->dev_info.rx_desc_lim.nb_mtu_seg_max;
...
static void init_config(void) {
  ...
  fwd_lcores = (fwd_lcore**)rte_zmalloc(
  "testpmd: fwd_lcores", sizeof(struct fwd_lcore*) * nb_lcores, RTE_CACHE_LINE_SIZE);
  if (fwd_lcores == NULL) {
    rte_exit(EXIT_FAILURE,
    "rte_zmalloc(%d (struct fwd_lcore *)) "
    "failed\n",
    nb_lcores);
  }
  for (lc_id = 0; lc_id < nb_lcores; lc_id++) {
    fwd_lcores[lc_id] =
    (fwd_lcore*)rte_zmalloc("testpmd: struct fwd_lcore", sizeof(struct fwd_lcore), RTE_CACHE_LINE_SIZE);
    if (fwd_lcores[lc_id] == NULL) {
      rte_exit(EXIT_FAILURE,
      "rte_zmalloc(struct fwd_lcore) "
      "failed\n");
    }
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    if (numa_support) {
      ...
      socket_id = (socket_num == UMA_NO_CONFIG) ? 0 : socket_num;
    }
  ...
  if (param_total_num_mbufs)
    ...
    nb_mbuf_per_pool = RX_DESC_MAX + (nb_lcores * mb_mempool_cache) + TX_DESC_MAX + MAX_PKT_BURST;
  ...
  if (numa_support) {
    ...
    for (i = 0; i < num_sockets; i++)
      for (j = 0; j < mbuf_data_size_n; j++)
        ...
        mbuf_pool_create(mbuf_data_size[j], nb_mbuf_per_pool, socket_ids[i], j);
  } else {
    uint8_t i;

    for (i = 0; i < mbuf_data_size_n; i++)
      mempools[i] = mbuf_pool_create(
      mbuf_data_size[i], nb_mbuf_per_pool, socket_num == UMA_NO_CONFIG ? 0 : socket_num, i);
  }
   ...
   */
  for (lc_id = 0; lc_id < nb_lcores; lc_id++) {
    mbp = mbuf_pool_find(rte_lcore_to_socket_id(fwd_lcores_cpuids[lc_id]), 0);
    ...
    fwd_lcores[lc_id]->gso_ctx.gso_size      = RTE_ETHER_MAX_LEN - RTE_ETHER_CRC_LEN;
  ...
  for (lc_id = 0; lc_id < nb_lcores; lc_id++) {
    gro_param.socket_id        = rte_lcore_to_socket_id(fwd_lcores_cpuids[lc_id]);
...
int init_fwd_streams(void) {
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    if (nb_rxq > port->dev_info.max_rx_queues) {
      fprintf(stderr, "Fail: nb_rxq(%d) is greater than max_rx_queues(%d)\n", nb_rxq, port->dev_info.max_rx_queues);
      return -1;
    }
    if (nb_txq > port->dev_info.max_tx_queues) {
      fprintf(stderr, "Fail: nb_txq(%d) is greater than max_tx_queues(%d)\n", nb_txq, port->dev_info.max_tx_queues);
      return -1;
    }
    if (numa_support) {
      if (port_numa[pid] != NUMA_NO_CONFIG)
  ...
  if (nb_fwd_streams) {
    fwd_streams = (fwd_stream**)rte_zmalloc(
    "testpmd: fwd_streams", sizeof(struct fwd_stream*) * nb_fwd_streams, RTE_CACHE_LINE_SIZE);
    if (fwd_streams == NULL)
      rte_exit(EXIT_FAILURE,
      "rte_zmalloc(%d"
    ...
    for (sm_id = 0; sm_id < nb_fwd_streams; sm_id++) {
      fwd_streams[sm_id] = (fwd_stream*)rte_zmalloc("testpmd:"
                                                    " struct fwd_stream",
...
static void pkt_burst_stats_display(const char* rx_tx, struct pkt_burst_stats* pbs) {
  ...
  for (i = 0, sburst = 0, sburstp = 0; i < 4; i++) {
    ...
    burst_percent[i] = (double)burst_stats[i] / total_burst * 100;
    printf("%d%% of %d pkts + ", burst_percent[i], (int)pktnb_stats[i]);
    sburstp += burst_percent[i];
  }
}

static void fwd_stream_stats_display(streamid_t stream_id) {
  struct fwd_stream* fs;
  static const char* fwd_top_stats_border = "-------";

  fs = fwd_streams[stream_id];
  if ((fs->rx_packets == 0) && (fs->tx_packets == 0) && (fs->fwd_dropped == 0))
    return;
  printf("\n  %s Forward Stats for RX Port=%2d/Queue=%2d -> "
         "TX Port=%2d/Queue=%2d %s\n",
  fwd_top_stats_border, fs->rx_port, fs->rx_queue, fs->tx_port, fs->tx_queue, fwd_top_stats_border);
...
void fwd_stats_display(void) {
  ...
  for (sm_id = 0; sm_id < cur_fwd_config.nb_fwd_streams; sm_id++) {
    ...
    if (cur_fwd_config.nb_fwd_streams > cur_fwd_config.nb_fwd_ports) {
    ...
    ports_stats[fs->rx_port].rx_bad_outer_l4_csum += fs->rx_bad_outer_l4_csum;
    ports_stats[fs->rx_port].rx_bad_outer_ip_csum += fs->rx_bad_outer_ip_csum;
  ...
  for (i = 0; i < cur_fwd_config.nb_fwd_ports; i++) {
    ...
    if (ret != 0) {
      fprintf(stderr, "%s: Error: failed to get stats (port %u): %d", __func__, pt_id, ret);
      continue;
    }
    ...
    stats.opackets, tx_dropped, stats.opackets + tx_dropped);

    if (record_burst_stats) {
      if (ports_stats[pt_id].rx_stream)
        pkt_burst_stats_display("RX", &ports_stats[pt_id].rx_stream->rx_burst_stats);
      if (ports_stats[pt_id].tx_stream)
        pkt_burst_stats_display("TX", &ports_stats[pt_id].tx_stream->tx_burst_stats);
    }
  ...
  printf("  RX-packets: %-14" PRIu64 " RX-dropped: %-14" PRIu64 "RX-total: "
         ...
         "%-" PRIu64 "\n",
  total_recv, total_rx_dropped, total_recv + total_rx_dropped, total_xmit, total_tx_dropped,
  total_xmit + total_tx_dropped);
...
#define CYC_PER_MHZ 1E6
    if (total_recv > 0 || total_xmit > 0) {
      ...
      if (strcmp(cur_fwd_eng->fwd_mode_name, "txonly") == 0 || strcmp(cur_fwd_eng->fwd_mode_name, "flowgen") == 0)
        total_pkts = total_xmit;
      else
        total_pkts = total_recv;

      printf("\n  CPU cycles/packet=%.2F (busy cycles="
             "%" PRIu64 " / total %s packets=%" PRIu64 ") at %" PRIu64 " MHz Clock\n",
      (double)fwd_cycles / total_pkts, fwd_cycles, cur_fwd_eng->fwd_mode_name, total_pkts,
...
void fwd_stats_reset(void) {
  ...
  for (i = 0; i < cur_fwd_config.nb_fwd_ports; i++) {
    ...
    if (ret != 0)
      fprintf(stderr, "%s: Error: failed to clear stats (port %u):%d", __func__, pt_id, ret);
  }
  for (sm_id = 0; sm_id < cur_fwd_config.nb_fwd_streams; sm_id++) {
    struct fwd_stream* fs = fwd_streams[sm_id];
...
static void flush_fwd_rx_queues(void) {
  ...
  if (num_procs > 1) {
    printf("multi-process not support for flushing fwd Rx queues, skip the "
           "below lines and return.\n");
    return;
  }
  ...
  for (j = 0; j < 2; j++) {
    for (rxp = 0; rxp < cur_fwd_config.nb_fwd_ports; rxp++) {
      for (rxq = 0; rxq < nb_rxq; rxq++) {
        ...
        if (ports[port_id].rxq[rxq].state == RTE_ETH_QUEUE_STATE_STOPPED)
          continue;
        ...
        do {
          ...
          timer_tsc += diff_tsc;
        } while ((nb_rx > 0) && (timer_tsc < timer_period));
        timer_tsc = 0;
      }
    }
...
void fnworker_dpdk(int core) {
  ...
  rte_mbuf* rx_mbufs[MAX_PKT_BURST];
  int num_worker_rings = nb_fwd_streams;
  while (1) {
    for (int lpid = 0; lpid < num_worker_rings; lpid++) {
      int dq = cne_ring_dequeue_bulk(worker_rx_rings_dpdk[lpid], (void**)rx_mbufs, nb_pkt_per_burst, NULL);
...
bool master_worker(struct fwd_stream* fs) {
  ...
  cne_ring_enqueue_bulk(worker_rx_rings_dpdk[fs->rx_port * +nb_cfg_ports + fs->rx_queue],
  (void**)(pkts_burst), nb_rx, NULL);

  unsigned int available = 0;
  do {
    unsigned int dq = cne_ring_dequeue_burst(worker_tx_rings_dpdk[fs->rx_port * +nb_cfg_ports + fs->rx_queue],
    (void**)pkts_burst, nb_pkt_per_burst, &available);
    common_fwd_stream_transmit(fs, pkts_burst, dq);
  } while (available != 0);
  return true;
}
...
packet_fwd_t pkt_fwd) // packet_fwd_t pkt_fwd -- this is some function which runs
{
  ...
  /* TEvery lcore here is resonsible for handling its own streams in the fstream_array that means
   ...
   need to see what the user want to do at the transmit side A good way for user to send data is
   by only sending it to the same queue on whihc its has received the data from.*/
  do {
...
#ifdef RTE_LIB_BITRATESTATS
    if (bitrate_enabled != 0 && bitrate_lcore_id == rte_lcore_id()) {
      tics_current = rte_rdtsc();
      if (tics_current - tics_datum >= tics_per_1sec) {
        /* Periodic bitrate calculation */
        for (i = 0; i < cnt_ports; i++)
          rte_stats_bitrate_calc(bitrate_data, ports_ids[i]);
        tics_datum = tics_current;
      }
    }
#endif
#ifdef RTE_LIB_LATENCYSTATS
    if (latencystats_enabled != 0 && latencystats_lcore_id == rte_lcore_id())
...
static int lcore_usage_callback(unsigned int lcore_id, struct rte_lcore_usage* usage) {
  struct fwd_stream** fsm;
  struct fwd_lcore* fc;
  streamid_t nb_fs;
  streamid_t sm_id;
...
static int start_pkt_forward_on_core(void* fwd_arg) {
  run_pkt_fwd_on_lcore((struct fwd_lcore*)fwd_arg, cur_fwd_config.fwd_eng->packet_fwd);
  return 0;
}
 ...
 */
static void launch_packet_forwarding(lcore_function_t* pkt_fwd_on_lcore) {
  unsigned int i;
  unsigned int lc_id;
  int diag;
  ...
  for (i = 0; i < cur_fwd_config.nb_fwd_lcores; i++) {
    ...
    if ((interactive == 0) || (lc_id != rte_lcore_id())) {
      ...
      diag                   = rte_eal_remote_launch(pkt_fwd_on_lcore, fwd_lcores[i], lc_id);
      if (diag != 0)
...
static void update_rx_queue_state(uint16_t port_id, uint16_t queue_id) {
  struct rte_eth_rxq_info rx_qinfo;
  int32_t rc;

  rc = rte_eth_rx_queue_info_get(port_id, queue_id, &rx_qinfo);
  if (rc == 0) {
    ports[port_id].rxq[queue_id].state = rx_qinfo.queue_state;
  } else if (rc == -ENOTSUP) {
    /*
     ...
     */
    ports[port_id].rxq[queue_id].state = RTE_ETH_QUEUE_STATE_STARTED;
  } else {
    TESTPMD_LOG(WARNING, "Failed to get rx queue info\n");
  }
}

static void update_tx_queue_state(uint16_t port_id, uint16_t queue_id) {
  struct rte_eth_txq_info tx_qinfo;
  int32_t rc;

  rc = rte_eth_tx_queue_info_get(port_id, queue_id, &tx_qinfo);
  if (rc == 0) {
    ports[port_id].txq[queue_id].state = tx_qinfo.queue_state;
  } else if (rc == -ENOTSUP) {
    /*
     ...
     */
    ports[port_id].txq[queue_id].state = RTE_ETH_QUEUE_STATE_STARTED;
  } else {
    TESTPMD_LOG(WARNING, "Failed to get tx queue info\n");
  }
}

static void update_queue_state(portid_t pid) {
  portid_t pi;
  queueid_t qi;
...
void start_packet_forwarding(int with_tx_first) {
  ...
  if (!nb_rxq || !nb_txq) {
    rte_exit(EXIT_FAILURE, "Either rxq or txq are 0, cannot use %s fwd mode\n", cur_fwd_eng->fwd_mode_name);
  }
  ...
  if (port_fwd_begin != NULL) {
    for (i = 0; i < cur_fwd_config.nb_fwd_ports; i++) {
      if (port_fwd_begin(fwd_ports_ids[i])) {
        fprintf(stderr, "Packet forwarding is not ready\n");
        return;
      }
    }
  }
...
static int all_ports_started(void) {
  ...
  RTE_ETH_FOREACH_DEV(pi) {
    ...
    if ((port->port_status != RTE_PORT_STARTED) && (port->member_flag == 0))
      return 0;
  }
...
int port_is_stopped(portid_t port_id) {
  ...
  if ((port->port_status != RTE_PORT_STOPPED) && (port->member_flag == 0))
    return 0;
  return 1;
}
...
static int setup_hairpin_queues(portid_t pi, portid_t p_pi, uint16_t cnt_pi) {
  ...
  if (!(hairpin_mode & 0xf)) {
    ...
    if (cnt_pi & 0x1) {
      ...
      peer_rx_port = rte_eth_find_next_owned_by(pi + 1, RTE_ETH_DEV_NO_OWNER);
  ...
  for (qi = nb_txq, i = 0; qi < nb_hairpinq + nb_txq; qi++) {
    ...
    if (port->port_status == RTE_PORT_HANDLING)
      ...
      fprintf(stderr, "Port %d can not be set back to stopped\n", pi);
...
struct rte_mempool* mp) {
  ...
  if ((rx_pkt_nb_segs > 1) && (rx_conf->offloads & RTE_ETH_RX_OFFLOAD_BUFFER_SPLIT)) {
    ...
    if (dev_info.max_rx_mempools == 0) {
      fprintf(stderr, "Port %u doesn't support requested multi-rx-mempool configuration.\n", port_id);
      return -ENOTSUP;
    }
    for (i = 0; i < mbuf_data_size_n; i++) {
  ...
  rx_conf->rx_deferred_start ? RTE_ETH_QUEUE_STATE_STOPPED : RTE_ETH_QUEUE_STATE_STARTED;
  return ret;
}

static int alloc_xstats_display_info(portid_t pi) {
  ...
  *ids_supp = (uint64_t*)calloc(xstats_display_num, sizeof(**ids_supp));
  if (*ids_supp == NULL)
    goto fail_ids_supp;

  *prev_values = (uint64_t*)calloc(xstats_display_num, sizeof(**prev_values));
  if (*prev_values == NULL)
    goto fail_prev_values;

  *curr_values = (uint64_t*)calloc(xstats_display_num, sizeof(**curr_values));
  if (*curr_values == NULL)
...
static void fill_xstats_display_info_for_port(portid_t pi) {
  ...
  if (!port->xstats_info.allocated && alloc_xstats_display_info(pi) != 0)
    rte_exit(EXIT_FAILURE, "Failed to allocate xstats display memory\n");

  ids_supp = port->xstats_info.ids_supp;
  for (stat = stat_supp = 0; stat < xstats_display_num; stat++) {
    xstat_name = xstats_display[stat].name;
    rc         = rte_eth_xstats_get_id_by_name(pi, xstat_name, ids_supp + stat_supp);
    if (rc != 0) {
...
int start_port(portid_t pid) {
  ...
  RTE_ETH_FOREACH_DEV(pi) {
    ...
    if (port->need_reconfig > 0) {
      ...
      if (flow_isolate_all) {
        ...
        if (ret) {
          fprintf(stderr, "Failed to apply isolated mode on port %d\n", pi);
          return -1;
        }
      }
      configure_rxtx_dump_callbacks(0);
      printf("Configuring Port %d (socket %u)\n", pi, port->socket_id);
      if (nb_hairpinq > 0 && rte_eth_dev_hairpin_capability_get(pi, &cap)) {
        fprintf(stderr, "Port %d doesn't support hairpin queues\n", pi);
        return -1;
      }

      /* configure port */
      diag = eth_dev_configure_mp(pi, nb_rxq + nb_hairpinq, nb_txq + nb_hairpinq, &(port->dev_conf));
      if (diag != 0) {
        if (port->port_status == RTE_PORT_HANDLING)
          port->port_status = RTE_PORT_STOPPED;
        else
      ...
      if (0 != eth_dev_conf_get_print_err(pi, &dev_conf)) {
        fprintf(stderr, "port %d can not get device configuration\n", pi);
        return -1;
      }
      /* Apply Rx offloads configuration */
      if (dev_conf.rxmode.offloads != port->dev_conf.rxmode.offloads) {
        ...
        for (k = 0; k < port->dev_info.max_rx_queues; k++)
          port->rxq[k].conf.offloads |= dev_conf.rxmode.offloads;
      }
      /* Apply Tx offloads configuration */
      if (dev_conf.txmode.offloads != port->dev_conf.txmode.offloads) {
        port->dev_conf.txmode.offloads |= dev_conf.txmode.offloads;
        for (k = 0; k < port->dev_info.max_tx_queues; k++)
          port->txq[k].conf.offloads |= dev_conf.txmode.offloads;
      }
    }
    if (port->need_reconfig_queues > 0 && is_proc_primary()) {
      ...
      /* setup tx queues */
      for (qi = 0; qi < nb_txq; qi++) {
        struct rte_eth_txconf* conf = &port->txq[qi].conf;

        if ((numa_support) && (txring_numa[pi] != NUMA_NO_CONFIG))
          diag =
          rte_eth_tx_queue_setup(pi, qi, port->nb_tx_desc[qi], txring_numa[pi], &(port->txq[qi].conf));
        else
          diag =
          rte_eth_tx_queue_setup(pi, qi, port->nb_tx_desc[qi], port->socket_id, &(port->txq[qi].conf));

        if (diag == 0) {
          port->txq[qi].state = conf->tx_deferred_start ? RTE_ETH_QUEUE_STATE_STOPPED : RTE_ETH_QUEUE_STATE_STARTED;
          continue;
        }
        ...
        if (port->port_status == RTE_PORT_HANDLING)
          ...
          fprintf(stderr, "Port %d can not be set back to stopped\n", pi);
        fprintf(stderr, "Fail to configure port %d tx queues\n", pi);
        /* try to reconfigure queues next time */
        port->need_reconfig_queues = 1;
        return -1;
      }
      for (qi = 0; qi < nb_rxq; qi++) {
        /* setup rx queues */
        if ((numa_support) && (rxring_numa[pi] != NUMA_NO_CONFIG)) {
          struct rte_mempool* mp = mbuf_pool_find(rxring_numa[pi], 0);
          if (mp == NULL) {
          ...
          diag = rx_queue_setup(pi, qi, port->nb_rx_desc[qi], rxring_numa[pi], &(port->rxq[qi].conf), mp);
        } else {
          struct rte_mempool* mp = mbuf_pool_find(port->socket_id, 0);
          if (mp == NULL) {
            fprintf(stderr, "Failed to setup RX queue: No mempool allocation on the socket %d\n",
            port->socket_id);
            return -1;
          }
          diag = rx_queue_setup(pi, qi, port->nb_rx_desc[qi], port->socket_id, &(port->rxq[qi].conf), mp);
        }
        if (diag == 0)
          continue;
        ...
        if (port->port_status == RTE_PORT_HANDLING)
          ...
          fprintf(stderr, "Port %d can not be set back to stopped\n", pi);
        fprintf(stderr, "Fail to configure port %d rx queues\n", pi);
        /* try to reconfigure queues next time */
        port->need_reconfig_queues = 1;
        return -1;
      }
    ...
    if (clear_ptypes) {
      ...
      if (diag < 0)
        fprintf(stderr, "Port %d: Failed to disable Ptype parsing\n", pi);
    }
    ...
    if (eth_macaddr_get_print_err(pi, &port->eth_addr) == 0)
      printf("Port %d: " RTE_ETHER_ADDR_PRT_FMT "\n", pi, RTE_ETHER_ADDR_BYTES(&port->eth_addr));
  ...
  if (hairpin_mode & 0xf) {
    ...
    for (i = 0; i < cfg_pi; i++) {
      ...
      peer_pi = rte_eth_hairpin_get_peer_ports(pi, peer_pl, RTE_MAX_ETHPORTS, 1);
      if (peer_pi < 0)
        return peer_pi;
      for (j = 0; j < peer_pi; j++) {
        ...
        if (diag < 0) {
          fprintf(stderr, "Error during binding hairpin Tx port %u to %u: %s\n", pi, peer_pl[j],
          rte_strerror(-diag));
          return -1;
        }
      }
      /* bind all peer Tx to current Rx */
      peer_pi = rte_eth_hairpin_get_peer_ports(pi, peer_pl, RTE_MAX_ETHPORTS, 0);
      if (peer_pi < 0)
        return peer_pi;
      for (j = 0; j < peer_pi; j++) {
        if (!port_is_started(peer_pl[j]))
...
void stop_port(portid_t pid) {
  ...
  RTE_ETH_FOREACH_DEV(pi) {
    ...
    if (port_is_forwarding(pi) != 0) {
      fprintf(stderr, "Please remove port %d from forwarding configuration.\n", pi);
      continue;
    }
...
static void remove_invalid_ports_in(portid_t* array, portid_t* total) {
  portid_t i;
...
void attach_port(char* identifier) {
  ...
  if (setup_on_probe_event) {
    ...
    for (pi = 0; pi < RTE_MAX_ETHPORTS; pi++)
      if (ports[pi].port_status == RTE_PORT_HANDLING && ports[pi].need_setup != 0)
...
void detach_port_device(portid_t port_id) {
  ...
  if (ret != 0) {
    TESTPMD_LOG(ERR, "Failed to get device info for port %d, not detaching\n", port_id);
    return;
  }
...
static int register_eth_event_callback(void) {
  int ret;
  int event;

  for (event = RTE_ETH_EVENT_UNKNOWN; event < RTE_ETH_EVENT_MAX; event++) {
    ret = rte_eth_dev_callback_register(
    RTE_ETH_ALL, static_cast<rte_eth_event_type>(event), eth_event_callback, NULL);
    if (ret != 0) {
      TESTPMD_LOG(ERR,
...
static int unregister_eth_event_callback(void) {
  int ret;
  int event;

  for (event = RTE_ETH_EVENT_UNKNOWN; event < RTE_ETH_EVENT_MAX; event++) {
    ret = rte_eth_dev_callback_unregister(
    RTE_ETH_ALL, static_cast<rte_eth_event_type>(event), eth_event_callback, NULL);
    if (ret != 0) {
      TESTPMD_LOG(ERR,
      "Failed to unregister callback for "
      "%s event\n",
      eth_event_desc[event]);
      return -1;
    }
...
/* This function is used by the interrupt thread */
static void dev_event_callback(const char* device_name, enum rte_dev_event_type type, __rte_unused void* arg) {
  uint16_t port_id;
  int ret;
...
static void rxtx_port_config(portid_t pid) {
  ...
  for (qid = 0; qid < nb_rxq; qid++) {
    ...
    if (rxq_share > 0 && (port->dev_info.dev_capa & RTE_ETH_DEV_CAPA_RXQ_SHARE)) {
    ...
    tx_buffer[pid][qid] = (rte_eth_dev_tx_buffer*)rte_zmalloc_socket(
    "tx_buffer", RTE_ETH_TX_BUFFER_SIZE(nb_pkt_per_burst), 0, rte_eth_dev_socket_id(pid));
    if (tx_buffer[pid] == NULL)
      rte_exit(EXIT_FAILURE, "Cannot allocate buffer for tx on port %u\n", pid);
    rte_eth_tx_buffer_init(tx_buffer[pid][qid], nb_pkt_per_burst);
  }
...
void init_port_config(void) {
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    if (nb_rxq > 1) {
      ...
      port->dev_conf.rx_adv_conf.rss_conf.rss_hf  = rss_hf & port->dev_info.flow_type_rss_offloads;
    } else {
      port->dev_conf.rx_adv_conf.rss_conf.rss_key = NULL;
    ...
    if (port->dcb_flag == 0) {
      if (port->dev_conf.rx_adv_conf.rss_conf.rss_hf != 0) {
        port->dev_conf.rxmode.mq_mode = (enum rte_eth_rx_mq_mode)(rx_mq_mode & RTE_ETH_MQ_RX_RSS);
      } else {
        port->dev_conf.rxmode.mq_mode = RTE_ETH_MQ_RX_NONE;
        port->dev_conf.rxmode.offloads &= ~RTE_ETH_RX_OFFLOAD_RSS_HASH;

        for (i = 0; i < port->dev_info.nb_rx_queues; i++)
          port->rxq[i].conf.offloads &= ~RTE_ETH_RX_OFFLOAD_RSS_HASH;
      }
    }
...
static void init_port(void) {
  ...
  ports = (rte_port*)rte_zmalloc("testpmd: ports", sizeof(struct rte_port) * RTE_MAX_ETHPORTS, RTE_CACHE_LINE_SIZE);
  if (ports == NULL) {
...
static void signal_handler(int signum __rte_unused) { f_quit = 1; }

char** create_eal_params(string str, int argc) {
  char** argv = new char*[argc];
  istringstream nss(str);
  string token;
  char* t = NULL;
  int i   = 0;
  while (nss >> token) {
    t = new char[token.size() + 1];
    memcpy(t, token.c_str(), token.size() + 1);
    argv[i] = t;
    i++;
  }
  return argv;
...
string count_args(Json::Value config) {
  string str = "./APPNAME " + config[LCORE_RELATED_OPTIONS_TAG].asString() + " " +
  config[MEMORY_REALTED_OPTIONS_TAG].asString() + " " + config[DEVICE_RELATED_OPTIONS_TAG].asString() +
  " " + config[MULTI_PROCESS_RELATED_OPTIONS_TAG].asString() + " " +
  config[DEBUGGIN_RELATED_OPTIONS_TAG].asString();

  str += " -- ";

  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(MP_ALLOC_TAG)) {
    str += "--" + std::string(MP_ALLOC_TAG) + "=" +
    config[QUEUES_AND_MBUF_OPTIONS_TAG][MP_ALLOC_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(MBUF_SIZE_TAG)) {
    str += "--" + std::string(MBUF_SIZE_TAG) + "=" +
    config[QUEUES_AND_MBUF_OPTIONS_TAG][MBUF_SIZE_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(TOTAL_NUM_MBUS_TAG)) {
    str += "--" + std::string(TOTAL_NUM_MBUS_TAG) + "=" +
    config[QUEUES_AND_MBUF_OPTIONS_TAG][TOTAL_NUM_MBUS_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(MBCACHE_TAG)) {
    str += "--" + std::string(MBCACHE_TAG) + "=" +
    config[QUEUES_AND_MBUF_OPTIONS_TAG][MBCACHE_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(MAX_PKT_LEN_TAG)) {
    str += "--" + std::string(MAX_PKT_LEN_TAG) + "=" +
    config[QUEUES_AND_MBUF_OPTIONS_TAG][MAX_PKT_LEN_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG].isMember(BURST_TAG)) {
    str += "--" + std::string(BURST_TAG) + "=" + config[QUEUES_AND_MBUF_OPTIONS_TAG][BURST_TAG].asString() + " ";
  }
  if (config[QUEUES_AND_MBUF_OPTIONS_TAG][NUM_RX_TX_DESC_TAG].asInt64() > 0) {
    str += "--rxd=" + config[QUEUES_AND_MBUF_OPTIONS_TAG][NUM_RX_TX_DESC_TAG].asString() + " ";
    str += "--txd=" + config[QUEUES_AND_MBUF_OPTIONS_TAG][NUM_RX_TX_DESC_TAG].asString() + " ";
  }


  int num_cores = 0;
  int num_ports = 0;
  auto it       = config["lports"].begin();
  while (it != config["lports"].end()) {
    num_ports++;
    num_cores += (*it).size();
    it++;
  }

  str += "--" + std::string(NB_CORES_TAG) + "=" + to_string(num_cores) + " ";
  str += "--" + std::string(NB_PORTS_TAG) + "=" + to_string(num_ports) + " ";
...
uint16_t DPDKCallBackWrapper::GetHeadRoom(void* buff) {
  return rte_pktmbuf_headroom(reinterpret_cast<rte_mbuf*>(buff));
}
uint16_t DPDKCallBackWrapper::GetTailRoom(void* buff) {
  return rte_pktmbuf_tailroom(reinterpret_cast<rte_mbuf*>(buff));
}
uint16_t DPDKCallBackWrapper::GetDataLength(void* buff) {
  return rte_pktmbuf_data_len(reinterpret_cast<rte_mbuf*>(buff));
}
char* DPDKCallBackWrapper::GetDataAtOffset(void* buff, uint16_t offset) {
  return rte_pktmbuf_mtod_offset(reinterpret_cast<rte_mbuf*>(buff), char*, offset);
}
char* DPDKCallBackWrapper::PrependData(void* buff, uint16_t offset) {
  return rte_pktmbuf_prepend(reinterpret_cast<rte_mbuf*>(buff), offset);
}
char* DPDKCallBackWrapper::AppendData(void* buff, uint16_t offset) {
  return rte_pktmbuf_append(reinterpret_cast<rte_mbuf*>(buff), offset);
}
char* DPDKCallBackWrapper::AdjustOffset(void* buff, uint16_t offset) {
  return rte_pktmbuf_adj(reinterpret_cast<rte_mbuf*>(buff), offset);
}
void* DPDKCallBackWrapper::TrimPacket(void* buff, uint16_t offset) {
  if (rte_pktmbuf_trim(reinterpret_cast<rte_mbuf*>(buff), offset) < 0) {
    log_error("TrimPacket operation failed on buff of len:%d with offset:%d",
    rte_pktmbuf_data_len(reinterpret_cast<pktmbuf_t*>(buff)), offset);
    return NULL;
  }
  return buff;
}
...
int DataPlaneManager::start_dpdk_datapath() {
  ...
  char** argv = create_eal_params(args, argc);
  ...
  int lport_ind = 0;
  auto it       = datapath_configuration[LPORTS_TAG].begin();
  while (it != datapath_configuration[LPORTS_TAG].end()) {
    portid_t port_id;
    if (rte_eth_dev_get_port_by_name(it.key().asCString(), &port_id) < 0) {
  ...
  if (!datapath_configuration.isMember(LPORTS_TAG)) {
    log_error("Required Parameter %s was not found in configuration file", LPORTS_TAG);
  }
  ...
  if (nb_rxq > 1 && nb_rxq > nb_txq)
    fprintf(stderr, "Warning: nb_rxq=%d enables RSS configuration, but nb_txq=%d will prevent to fully test it.\n",
    nb_rxq, nb_txq);
  ...
  if (dpdk_dataplane_manager->get_model() == MODEL_VALUE_MASTER_WORKER) {
    ...
    for (int i = 0; i < nb_fwd_streams; i++) {
      ...
      worker_tx_rings_dpdk[i] = cne_ring_create("worker_tx_ring", sizeof(void*), ring_size, 0);
    }
    if (!worker_configuration["worker-cores"].isArray()) {
      log_error("Inavlid Configuration: worker-cores should be an array\n");
    }
    auto it = workers_config["worker-cores"].begin();
    while (it != workers_config["worker-cores"].end()) {
      if ((*it).isInt()) {
        workers_cores.push_back((*it).asUInt());
      } else if ((*it).isString()) {
        ...
        for (int i = start; i <= end; i++)
          workers_cores.push_back(i);
      }
      it++;
    }
    cur_fwd_eng = &master_worker_engine;
    workers.resize(workers_cores.size());
    for (int i = 0; i < workers_cores.size(); i++)
      workers[i] = thread(fnworker_dpdk, workers_cores[i]);
  }
