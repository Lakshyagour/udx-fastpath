...
/* Group types */
{"all", RTE_ETH_RSS_ETH | RTE_ETH_RSS_VLAN | RTE_ETH_RSS_IP | RTE_ETH_RSS_TCP | RTE_ETH_RSS_UDP | RTE_ETH_RSS_SCTP | RTE_ETH_RSS_L2_PAYLOAD | RTE_ETH_RSS_L2TPV3 | RTE_ETH_RSS_ESP | RTE_ETH_RSS_AH | RTE_ETH_RSS_PFCP | RTE_ETH_RSS_GTPU | RTE_ETH_RSS_ECPRI | RTE_ETH_RSS_MPLS | RTE_ETH_RSS_L2TPV2},
{"none", 0},
{"ip", RTE_ETH_RSS_IP},
{"udp", RTE_ETH_RSS_UDP},
{"tcp", RTE_ETH_RSS_TCP},
...
nic_xstats_display_periodic(portid_t port_id) {
  ...
  for (i = 0; i < ids_supp_sz; i++) {
    ...
    value_rate     = diff_ns > 0 ?
        (double)diff_value / diff_ns * NS_PER_SEC :
        0;
...
void nic_stats_display(portid_t port_id) {
  ...
  mpps_rx               = diff_ns > 0 ?
                (double)diff_pkts_rx / diff_ns * NS_PER_SEC :
                0;
  mpps_tx               = diff_ns > 0 ?
                (double)diff_pkts_tx / diff_ns * NS_PER_SEC :
                0;
  ...
  mbps_rx                = diff_ns > 0 ?
                 (double)diff_bytes_rx / diff_ns * NS_PER_SEC :
                 0;
  mbps_tx                = diff_ns > 0 ?
                 (double)diff_bytes_tx / diff_ns * NS_PER_SEC :
                 0;
...
static int
action_alloc(portid_t port_id, uint32_t id, struct port_indirect_action** action) {
  struct rte_port* port;
  struct port_indirect_action** ppia;
  struct port_indirect_action* pia = NULL;
...
static int
template_alloc(uint32_t id, struct port_template** template, struct port_template** list) {
...
static int
table_alloc(uint32_t id, struct port_table** table, struct port_table** list) {
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pia->handle) {
        ...
        rte_flow_action_list_handle_destroy(port_id, pia->list_handle, &error) :
...
int port_action_handle_update(portid_t port_id, uint32_t id, const struct rte_flow_action* action) {
  struct rte_flow_error error;
...
void port_action_handle_query_update(portid_t port_id, uint32_t id, enum rte_flow_query_update_mode qu_mode, const struct rte_flow_action* action) {
  int ret;
  struct rte_flow_error error;
  struct port_indirect_action* pia;
  union port_action_query query;
...
int port_meter_policy_add(portid_t port_id, uint32_t policy_id, const struct rte_flow_action* actions) {
...
int port_flow_pattern_template_create(portid_t port_id, uint32_t id, const struct rte_flow_pattern_template_attr* attr, const struct rte_flow_item* pattern) {
  struct rte_port* port;
  ...
  /* Poisoning to make sure PMDs update it in case of error. */
  memset(&error, 0x22, sizeof(error));
  pit->template_t.pattern_template = rte_flow_pattern_template_create(port_id,
  attr, pattern, &error);
  if (!pit->template_t.pattern_template) {
...
int port_flow_pattern_template_destroy(portid_t port_id, uint32_t n, const uint32_t* template) {
  struct rte_port* port;
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      pit->template_t.pattern_template,
      &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
...
int port_flow_pattern_template_flush(portid_t port_id) {
  ...
  while (*tmp) {
    ...
    if (pit->template_t.pattern_template &&
    rte_flow_pattern_template_destroy(port_id,
    pit->template_t.pattern_template, &error)) {
...
int port_flow_actions_template_create(portid_t port_id, uint32_t id, const struct rte_flow_actions_template_attr* attr, const struct rte_flow_action* actions, const struct rte_flow_action* masks) {
  struct rte_port* port;
  ...
  /* Poisoning to make sure PMDs update it in case of error. */
  memset(&error, 0x22, sizeof(error));
  pat->template_t.actions_template = rte_flow_actions_template_create(port_id,
  attr, actions, masks, &error);
  if (!pat->template_t.actions_template) {
...
int port_flow_actions_template_destroy(portid_t port_id, uint32_t n, const uint32_t* template) {
  struct rte_port* port;
  struct port_template** tmp;
  int ret = 0;
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      pat->template_t.actions_template, &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
      *tmp = pat->next;
...
/** Create table */
int port_flow_template_table_create(portid_t port_id, uint32_t id, const struct rte_flow_template_table_attr* table_attr, uint32_t nb_pattern_templates, uint32_t* pattern_templates, uint32_t nb_actions_templates, uint32_t* actions_templates) {
  struct rte_port* port;
  ...
  uint32_t i;
  struct rte_flow_error error;
  struct rte_flow_pattern_template* flow_pattern_templates[nb_pattern_templates];
  struct rte_flow_actions_template* flow_actions_templates[nb_actions_templates];
  ...
  for (i = 0; i < nb_pattern_templates; ++i) {
    ...
    while (temp) {
      if (pattern_templates[i] == temp->id) {
        ...
        temp->template_t.pattern_template;
        found = true;
        break;
      }
      temp = temp->next;
  ...
  for (i = 0; i < nb_actions_templates; ++i) {
    ...
    while (temp) {
      if (actions_templates[i] == temp->id) {
        ...
        temp->template_t.actions_template;
        found = true;
        break;
      }
      temp = temp->next;
...
int port_queue_flow_create(portid_t port_id, queueid_t queue_id, bool postpone, uint32_t table_id, uint32_t rule_idx, uint32_t pattern_idx, uint32_t actions_idx, const struct rte_flow_item* pattern, const struct rte_flow_action* actions) {
...
int port_queue_flow_destroy(portid_t port_id, queueid_t queue_id, bool postpone, uint32_t n, const uint64_t* rule) {
  struct rte_flow_op_attr op_attr = {.postpone = postpone};
  struct rte_port* port;
...
static void
queue_action_handle_create(portid_t port_id, uint32_t queue_id, struct port_indirect_action* pia, struct queue_job* job, const struct rte_flow_op_attr* attr, const struct rte_flow_indir_action_conf* conf, const struct rte_flow_action* action, struct rte_flow_error* error) {
  if (action->type == RTE_FLOW_ACTION_TYPE_AGE) {
    struct rte_flow_action_age* age =
    (struct rte_flow_action_age*)(uintptr_t)(action->conf);
...
static void
queue_action_list_handle_create(portid_t port_id, uint32_t queue_id, struct port_indirect_action* pia, struct queue_job* job, const struct rte_flow_op_attr* attr, const struct rte_flow_indir_action_conf* conf, const struct rte_flow_action* action, struct rte_flow_error* error) {
  /* Poisoning to make sure PMDs update it in case of error. */
  pia->type        = RTE_FLOW_ACTION_TYPE_INDIRECT_LIST;
  pia->list_handle = rte_flow_async_action_list_handle_create(port_id, queue_id, attr, conf, action,
  job, error);
}

/** Enqueue update flow rule operation. */
int port_queue_flow_update(portid_t port_id, queueid_t queue_id, bool postpone, uint32_t rule_idx, uint32_t actions_idx, const struct rte_flow_action* actions) {
  struct rte_flow_op_attr op_attr = {.postpone = postpone};
  struct rte_port* port;
...
int port_queue_action_handle_create(portid_t port_id, uint32_t queue_id, bool postpone, uint32_t id, const struct rte_flow_indir_action_conf* conf, const struct rte_flow_action* action) {
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      ret       = pia->type == RTE_FLOW_ACTION_TYPE_INDIRECT_LIST ?
            rte_flow_async_action_list_handle_destroy(port_id, queue_id,
            &attr, pia->list_handle,
            job, &error) :
            rte_flow_async_action_handle_destroy(port_id, queue_id, &attr, pia->handle,
            job, &error);
      if (ret) {
        free(job);
        ret = port_flow_complain(&error);
...
int port_queue_action_handle_update(portid_t port_id,
uint32_t queue_id,
bool postpone,
uint32_t id,
const struct rte_flow_action* action) {
...
void port_queue_action_handle_query_update(portid_t port_id,
uint32_t queue_id,
bool postpone,
uint32_t id,
...
int port_queue_action_handle_query(portid_t port_id,
uint32_t queue_id,
bool postpone,
uint32_t id) {
  const struct rte_flow_op_attr attr = {.postpone = postpone};
...
int port_flow_hash_calc(portid_t port_id, uint32_t table_id, uint8_t pattern_template_index, const struct rte_flow_item pattern[]) {
  uint32_t hash;
  bool found;
  struct port_table* pt;
  struct rte_port* port;
...
static int
port_queue_aged_flow_destroy(portid_t port_id, queueid_t queue_id, const uint64_t* rule, int nb_flows) {
  struct rte_port* port = &ports[port_id];
...
int port_queue_group_set_miss_actions(portid_t port_id, const struct rte_flow_attr* attr, const struct rte_flow_action* actions) {
...
int port_flow_destroy(portid_t port_id, uint32_t n, const uint64_t* rule, bool is_user_id) {
  struct rte_port* port;
  struct port_flow** tmp;
  int ret = 0;
...
/** Dump flow rules. */
int port_flow_dump(portid_t port_id, bool dump_all, uint64_t rule_id, const char* file_name, bool is_user_id) {
  int ret    = 0;
  FILE* file = stdout;
  struct rte_flow_error error;
  struct rte_port* port;
...
/** Query a flow rule. */
int port_flow_query(portid_t port_id, uint64_t rule, const struct rte_flow_action* action, bool is_user_id) {
  struct rte_flow_error error;
  struct rte_port* port;
  struct port_flow* pf;
  const char* name;
...
void port_rss_hash_key_update(portid_t port_id, char rss_type[], uint8_t* hash_key, uint8_t hash_key_len) {
 ...
 */
static bool
fwd_stream_on_other_lcores(uint16_t domain_id, lcoreid_t src_lc, portid_t src_port, queueid_t src_rxq, uint32_t share_group, queueid_t share_rxq) {
  streamid_t sm_id;
  ...
  for (lc_id = src_lc + 1; lc_id < nb_fc; lc_id++) {
    ...
    for (; sm_id < fwd_lcores[lc_id]->stream_idx + nb_fs_per_lcore;
      ...
      if ((dev_info->dev_capa & RTE_ETH_DEV_CAPA_RXQ_SHARE) == 0 || rxq_conf->share_group == 0)
        /* Not shared rxq. */
        continue;
...
rss_fwd_config_setup(void) {
  ...
  for (sm_id = 0; sm_id < cur_fwd_config.nb_fwd_streams; sm_id++) {
    ...
    fs = fwd_streams[sm_id];
    // Lakshya :- need to set the topology to LOOPBACK, the user will give the verdict [port,queue]
...
static int
get_fdir_info(portid_t port_id, struct rte_eth_fdir_info* fdir_info, struct rte_eth_fdir_stats* fdir_stat) {
  int ret = -ENOTSUP;
...
void fdir_get_infos(portid_t port_id) {
  ...
  if (fdir_info.mode == RTE_FDIR_MODE_PERFECT)
    ...
    printf("  DISABLE\n");
  if (fdir_info.mode != RTE_FDIR_MODE_PERFECT_MAC_VLAN && fdir_info.mode != RTE_FDIR_MODE_PERFECT_TUNNEL) {
...
int set_vf_rate_limit(portid_t port_id, uint16_t vf, uint32_t rate, uint64_t q_msk) {
  int diag = -ENOTSUP;
...
int set_rxq_avail_thresh(portid_t port_id, uint16_t queue_id, uint8_t avail_thresh) {
  if (port_id_is_invalid(port_id, ENABLED_WARN))
    return -EINVAL;
...
mcast_addr_pool_extend(struct rte_port* port) {
   ...
   * of MCAST_POOL_INC.
   */
  mc_pool_size = sizeof(struct rte_ether_addr) * (port->mc_addr_nb + MCAST_POOL_INC);
